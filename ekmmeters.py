...
def calc_crc16_as_bytes(buf):
    return a2b_hex(calc_crc16(buf))
...
class SerialPort(object):
    ...
    __enter__ and __exit__ functions defined so this class can become 
    a context manager and thus be used in a "with" statement.
    """
    ...
    def initPort(self):
        ...
        # If already inited, just return True
        if self.m_ser is not None:
            return True
        try:
            try:
                ...
                # if the port can't be opened with Serial, try serial_for_url
                if e.errno == 2:  self.m_ser = serial.serial_for_url(url=self.m_ttyport,
                                        baudrate=self.m_baudrate,
                                        timeout=0,
                                        parity=serial.PARITY_EVEN,
                                        stopbits=serial.STOPBITS_ONE,
    ...
    def closePort(self):
        ...
        # If not opened (inited) just return
        if self.m_ser is None:
            return
        self.m_ser.close()
        self.m_ser = None
        pass
    ...
    def write(self, output):
        ...
        if isinstance(output, str):
            try:
                view_str = a2b_hex(output)
            except binascii.Error as e:
                ekm_log('SerialPort.write: output string has %s'%e, priority=2)
                raise
        elif isinstance(output, bytearray) or isinstance(output, bytes):
            view_str = output
        else:
            ekm_log('SerialPort.write called with unknown argument class.')
            return False
        ekm_log('SerialPort:  Writing bytes to port: %s'%view_str, priority=5)
        if (len(view_str) > 0):
    ...
    def getResponse(self, context="", maxBytes=255):
        ...
        response_str = bytearray()  # returned bytes in string default
        try:
            waits = 0  # allowed interval counter
            while (waits < self.m_max_waits):
                bytes_to_read = self.m_ser.inWaiting()
                # ekm_log('%s bytes have arrived.'%bytes_to_read)
                if bytes_to_read > 0:
                    next_chunk = self.m_ser.read(bytes_to_read)
                    response_str = bytearray().join((response_str,next_chunk))
                    # ekm_log('Msg chunk recvd: "%s", total msg so far: "%s" (%d bytes)'%(next_chunk, response_str, len(response_str)))
                    if (len(response_str) == maxBytes):
                        time.sleep(self.m_force_wait)
                        ekm_log('Received message is: %s'%response_str)
                        return response_str.decode('ascii')
                    #  1 byte responses from the meter are either ACK (= 0x06) or NAK (= 0x15)
                    if (len(response_str) == 1) and ((response_str[0] == 0x06) or (response_str[0] == 0x15)):
                        time.sleep(self.m_force_wait)
                        ekm_log('ACK/NAK received message is: %s'%response_str)
                        return b2a_hex(response_str).decode('ascii')
                    #  else keep waiting
                else:  # hang out -- half shortest expected interval (50 ms)
                    waits += 1
                    time.sleep(self.m_force_wait)
            #  timed out -- throw away any received bytes and return empty string
            ekm_log('No complete message received before timeout.  Received so far: "%s"'%response_str)
            response_str = ""
...
class Meter(object):
    ...
    def __init__(self, meter_address="000000000000", serial_port=None):
        """
        Args:
    ...
    def openMeter(self):        # Never sends termination string
        """ Required override, issue A read to initiate communication with meter.
        ...
        Returns:
            bool: True on successful read.
        """
        ekm_log("Meter::openMeter called in superclass.")
        return False
    ...
    def setMaxDemandPeriod(self, period, password="00000000"):
        ...
        try:
            ...
            if not self.openMeter():
                self.writeCmdMsg("Initiate connection to meter failed.")
            else:
                if not self.serialCmdPwdAuth(password):
                    self.writeCmdMsg("Password failure")
                else:
                    req_str = "015731023030353028" + str(period).zfill(2).encode().hex() + "2903"
                    req_str += self.calc_crc16(a2b_hex(req_str[2:]))
                    self.m_serial_port.write(a2b_hex(req_str))
                    if self.m_serial_port.getResponse(self.getContext()) == "06":
        ...
        try:
            ...
            if not self.openMeter():
                ...
                if not self.serialCmdPwdAuth(password):
                    ...
                    req_str = "015731023030443528" + str(interval).zfill(1).encode().hex() + "2903"
                    req_str += self.calc_crc16(a2b_hex(req_str[2:]))
                    self.m_serial_port.write(a2b_hex(req_str))
                    if self.m_serial_port.getResponse(self.getContext()) == "06":
        ...
        try:
            ...
            if not self.openMeter():
                ...
                if not self.serialCmdPwdAuth(pwd):
                    ...
                    req_pwd = new_pwd.zfill(8).encode().hex()
                    ...
                    if self.m_serial_port.getResponse(self.getContext()) == "06":
                        ...
                        self.writeCmdMsg("Failure(setMeterPassword): Not 06 returned.")
                        result = False
    ...
    def unpackStruct(self, data, def_buf):
        ...
        if (struct.calcsize(struct_str) == 255) and (len(data) == 255):
            contents = struct.unpack(struct_str, data.encode('ascii'))
        else:
            if struct.calcsize(struct_str) != 255:
                self.writeCmdMsg('Structure format size is not 255 = expected data size.')
            if len(data) != 255:
    ...
    def crcMeterRead(self, raw_read, def_buf):
        ...
        try:
            if len(raw_read) == 0:
                ...
                return False
            sent_crc = self.calc_crc16(raw_read[1:-2].encode('ascii'))
        ...
        # errors.
            ...
            for frame in traceback.extract_tb(sys.exc_info()[2]):
                ...
                fn          # reference variables to silence warning.
                text
        ...
        try:
            ...
            if not self.openMeter():
                ...
                if not self.serialCmdPwdAuth(password):
                    ...
                    req_str = "015731023030343028" + str(0).zfill(6).encode().hex() + "2903"
                    req_str += self.calc_crc16(a2b_hex(req_str[2:]))
                    self.m_serial_port.write(a2b_hex(req_str))
                    if self.m_serial_port.getResponse(self.getContext()) == "06":
    ...
    def setTimeFromDateTime(self, theDateTime=datetime.datetime.now(), password="00000000"):
        return self.setTime(theDateTime.year, 
                            theDateTime.month,
                            theDateTime.day,
                            theDateTime.hour,
                            theDateTime.minute,
        ...
        try:
            ...
            if not self.openMeter():
                ...
                if not self.serialCmdPwdAuth(password):
                    ...
                    req_str += str(yy)[-2:].encode().hex()
                    req_str += str(mm).zfill(2).encode().hex()
                    req_str += str(dd).zfill(2).encode().hex()
                    req_str += str(dayofweek).zfill(2).encode().hex()
                    req_str += str(hh).zfill(2).encode().hex()
                    req_str += str(minutes).zfill(2).encode().hex()
                    req_str += str(ss).zfill(2).encode().hex()
                    req_str += "2903"
                    req_str += self.calc_crc16(a2b_hex(req_str[2:]))
                    self.m_serial_port.write(a2b_hex(req_str))
                    response = self.m_serial_port.getResponse(self.getContext())
                    if response == "06":
        ...
        try:
            ...
            if not self.openMeter():
                ...
                if not self.serialCmdPwdAuth(password):
                    ...
                    req_str = "015731023030443028" + str(new_ct).zfill(4).encode().hex() + "2903"
                    req_str += self.calc_crc16(a2b_hex(req_str[2:]))
                    self.m_serial_port.write(a2b_hex(req_str))
                    if self.m_serial_port.getResponse(self.getContext()) == "06":
        ...
        try:
            if not self.openMeter():
                ...
                if not self.serialCmdPwdAuth(password):
                    ...
                    req_table = ""
                    req_table += str(cmd_dict["Hour_1"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Min_1"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Rate_1"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Hour_2"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Min_2"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Rate_2"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Hour_3"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Min_3"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Rate_3"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Hour_4"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Min_4"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Rate_4"]).zfill(2).encode().hex()
                    req_table += str(0).zfill(24).encode().hex()

                    table = str(cmd_dict["Schedule"]).zfill(1).encode().hex()
        ...
        try:
            if not self.openMeter():
                ...
                if not self.serialCmdPwdAuth(password):
                    ...
                    req_table = ""
                    req_table += str(cmd_dict["Season_1_Start_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Season_1_Start_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Season_1_Schedule"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Season_2_Start_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Season_2_Start_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Season_2_Schedule"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Season_3_Start_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Season_3_Start_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Season_3_Schedule"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Season_4_Start_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Season_4_Start_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Season_4_Schedule"]).zfill(2).encode().hex()
                    req_table += str(0).zfill(24).encode().hex()
        ...
        try:
            if not self.openMeter():
                ...
                if not self.serialCmdPwdAuth(password):
                    ...
                    req_table = ""
                    req_table += str(cmd_dict["Holiday_1_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_1_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_2_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_2_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_3_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_3_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_4_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_4_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_5_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_5_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_6_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_6_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_7_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_7_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_8_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_8_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_9_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_9_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_10_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_10_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_11_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_11_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_12_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_12_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_13_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_13_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_14_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_14_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_15_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_15_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_16_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_16_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_17_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_17_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_18_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_18_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_19_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_19_Day"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_20_Month"]).zfill(2).encode().hex()
                    req_table += str(cmd_dict["Holiday_20_Day"]).zfill(2).encode().hex()
        ...
        try:
            if not self.openMeter():
                ...
                if not self.serialCmdPwdAuth(password):
                    ...
                    req_wkd = str(new_wknd).zfill(2).encode().hex()
                    req_hldy = str(new_hldy).zfill(2).encode().hex()
    ...
    def readScheduleTariffs(self, tableset):
        ...
        try:
            req_table = str(tableset).zfill(1).encode().hex()
            req_str = "01523102303037" + req_table + "282903"

            if not self.openMeter():
                self.writeCmdMsg("Initiate connection to meter failed.")
                return False
            req_crc = self.calc_crc16(a2b_hex(req_str[2:]))
            req_str += req_crc
            self.m_serial_port.write(a2b_hex(req_str))
    ...
    def readMonthTariffs(self, months_type):
        ...
        try:
            ...
            req_type = str(months_type).zfill(1).encode().hex()
    ...
    def serialCmdPwdAuth(self, password_str):
        ...
        result = False
        try:
            req_start = "0150310228" + password_str.encode().hex() + "2903"
            req_crc = self.calc_crc16(a2b_hex(req_start[2:]))
...
class V3Meter(Meter):
    ...
    def __init__(self, meter_address="000000000000", serial_port=None):
        ...
        super(V3Meter, self).__init__(meter_address, serial_port)
    ...
    def __exit__(self, type, value, traceback):
        if self.m_serial_port is not None:
            self.m_serial_port.closePort()
    ...
    def attachPort(self, serial_port):
        ...
        Args:
            serial_port (SerialPort): Serial port object, does need to be initialized.
        """
        if self.m_serial_port is not None:      #  Don't do anything if already attached
            return
        self.m_serial_port = serial_port
        pass
    ...
    def request(self, send_terminator = False):
        ...
        try:
            self.m_serial_port.write(a2b_hex("2f3f" +
                                     self.m_meter_address.encode('ascii').hex() +
                                     "210d0a"))
    ...
    def insert(self, meter_db):
        ...
        Picks up m_raw_read_b from super.
        """
        if meter_db:
    ...
    def serialPostEnd(self):
        ...
        self.m_serial_port.write(a2b_hex("0142300375"))
        pass
...
class V4Meter(Meter):
    ...
    def __init__(self, meter_address="000000000000", serial_port=None):
        ...
        super(V4Meter, self).__init__(meter_address, serial_port)
    ...
    def requestA(self):
        ...
        self.m_serial_port.write("2f3f" + self.m_meter_address.encode('ascii').hex() + "3030210d0a")
        self.m_raw_read_a = self.m_serial_port.getResponse(self.getContext())
        unpacked_read_a = self.unpackStruct(self.m_raw_read_a, self.m_blk_a)
    ...
    def requestB(self):
        ...
        self.m_serial_port.write(a2b_hex("2f3f" + self.m_meter_address.encode('ascii').hex() + "3031210d0a"))
    ...
    def makeAB(self):
        ...
        ## Put in fields from A read second so that duplicated fields will get data
        ## from A
        for fld in self.m_blk_a:
            compare_fld = fld.upper()
            if not "RESERVED" in compare_fld and not "CRC" in compare_fld:
    ...
    def getFieldA(self, fld_name):
        ...
        """
        result = ""
        if fld_name in self.m_blk_a:
            result = self.m_blk_a[fld_name][MeterData.StringValue]
        else:
            ekm_log("Requested nonexistent field: " + fld_name)

        return result

    def getFieldANative(self, fld_name):
        """ Return :class:`~ekmmeters.Field` content, scaled and formatted.
        ...
        result = None
        if fld_name in self.m_blk_a:
            result = self.m_blk_a[fld_name][MeterData.NativeValue]
        else:
            ekm_log("Requested nonexistent field: " + fld_name)

        return result

    def getFieldB(self, fld_name):
        """ Return :class:`~ekmmeters.Field` content, scaled and formatted.
        ...
        """
        result = ""
        if fld_name in self.m_blk_b:
            result = self.m_blk_b[fld_name][MeterData.StringValue]
        else:
            ekm_log("Requested nonexistent field: " + fld_name)
            ...
            if not self.openMeter():
                ...
                if not self.serialCmdPwdAuth(password):
                    ...
                    req_str = ("01573102303038" +
                               str(relay).encode().hex() +
                               "28" +
                               str(status).encode().hex() +
                               str(seconds).zfill(4).encode().hex() + "2903")
                    req_str += self.calc_crc16(a2b_hex(req_str[2:]))
                    self.m_serial_port.write(a2b_hex(req_str))
                    if self.m_serial_port.getResponse(self.getContext()).encode() == "06":
        ...
        try:
            if not self.openMeter():
                ...
                if not self.serialCmdPwdAuth(password):
                    ...
                    req_const = str(new_cnst).zfill(4).encode().hex()
                    line_const = str(line_in - 1).encode().hex()
        ...
        try:
            if not self.openMeter():
                ...
                if not self.serialCmdPwdAuth(password):
                    ...
                    req_str = "015731023030443428" + str(new_pout).zfill(4).encode().hex() + "2903"
                    req_str += self.calc_crc16(a2b_hex(req_str[2:]))
                    self.m_serial_port.write(a2b_hex(req_str))
                    if self.m_serial_port.getResponse(self.getContext()) == "06":
                        self.writeCmdMsg("Success: 06 returned.")
            ...
            if not self.openMeter():
                ...
                if not self.serialCmdPwdAuth(password):
                    ...
                    for lcdid in self.m_lcd_items:
                        append_val = str(lcdid).zfill(2).encode().hex()
                        req_table += append_val

                    for i in range(0, fill_len):
                        i       # reference to silence unused variable warning
                        append_val = str(0).zfill(2).encode().hex()
                        req_table += append_val
